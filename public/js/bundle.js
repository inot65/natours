...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoidGZ0LXRvbmkiLCJhIjoiY2t3cXM0eXQ5MHBlZTJ2cDM1YXZja3A2eiJ9.nWd0OWLkRGuu7xPWoJyKCQ';
  ...
  locations.forEach(function (loc) {
    ...
    el.className = 'marker'; // face un parcher specific

    new mapboxgl.Marker({
      element: el,
      anchor: 'bottom'
    }).setLngLat(loc.coordinates).addTo(map); // adaugam un pop-up

    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>day: ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // extinde limitele hartii ca sa includa locatiile
...
var showAlert = function showAlert(type, msg) {
  // ascund toate alertele care exista deja !
  hideAlert();
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 7:
            ...
            //console.log(err.response);
            (0, _alerts.showAlert)('error', 'Error logging oyt ! Try again.');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              (0, _alerts.showAlert)(res.data.status, 'Datele contului dvs. au fost actualizate!');
            } //console.log(res);
...
var stripe = Stripe('pk_test_51K7O1CBAYScTBNRfwkxPF8HDVDId1tB4nF8ufvSKLx1rrRE91lk1aF0Bi6nJRi0vnjDb6JTdzHBpgpBmUG6plrit00JmDslUev'); // folosim cheia PUBLICA aici
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            console.log(session); // 2. creat checckout form  + charge credit card
...
var bookBtn = document.getElementById('book-tour'); // valori
...
if (bookBtn) bookBtn.addEventListener('click', function (e) {
  ...
  e.target.textContent = 'Processing...'; // fiind aceasi denumire, folosim destructurarea !
